...
class RetinaMNISTDataset():
    def __init__(self, dataset_path: str, split: str = None, transform: Optional[transforms.Compose] = None):
        os.makedirs(dataset_path, exist_ok=True)
        self.dataset = RetinaMNIST(root=dataset_path, split=split, download=True, size=224)
        self.split = split
...
class RetinaMNISTDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_dataset = RetinaMNISTDataset(
                dataset_path=self.dataset_path,
                split='train',
                transform=self.train_transform,
            )
            self.val_dataset = RetinaMNISTDataset(
                dataset_path=self.dataset_path,
                split='val',
                transform=self.test_transform,
            )
        if stage == 'test' or stage is None:
            self.test_dataset = RetinaMNISTDataset(
                dataset_path=self.dataset_path,
                split='test',
                transform=self.test_transform,
            )
