...
class OxfordIIITPetDataset(Dataset):
    def __init__(self, dataset_path: str, split: str = None, transform: Optional[transforms.Compose] = None):
        ...
        if split in ('train', 'validation'):
            self.dataset = torch_datasets.OxfordIIITPet(dataset_path, split='trainval', download=True)
        ...
        if split == 'test':
            self.dataset = torch_datasets.OxfordIIITPet(dataset_path, split='test', download=True)
            self.targets = np.array(self.dataset._labels)
    ...
    def deterministic_train_val_split(self):
        self.train_idx, self.val_idx = train_test_split(
            self.train_val_idx, random_state=2024, test_size=0.15, train_size=0.85, stratify=self.targets
        )
...
class OxfordIIITPetDataModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_dataset = OxfordIIITPetDataset(
                dataset_path=self.dataset_path,
                split='train',
                transform=self.train_transform,
            )
            self.val_dataset = OxfordIIITPetDataset(
                dataset_path=self.dataset_path,
                split='validation',
                transform=self.test_transform,
            )
        if stage == 'test' or stage is None:
            self.test_dataset = OxfordIIITPetDataset(
                dataset_path=self.dataset_path,
                split='test',
                transform=self.test_transform,
            )
