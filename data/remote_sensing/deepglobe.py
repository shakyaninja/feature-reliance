...
class DeepGlobeDataset(Dataset):
    ...
    def extract_multilabel_from_imgpath(self, path):
        """
        Extracts the multi-label vector from a filename like 'image016519_000110.jpg'
        """
        filename = os.path.basename(path)
        label_number = filename.split('_')[1].split('.')[0]
        multi_label = np.zeros(6)
        for i, char in enumerate(label_number):
            multi_label[i] = int(char)
        multi_label = multi_label.astype(np.float32)
        return multi_label
    ...
    def read_image_paths(self):
        for ext in ('*.jpg', '*.png', '*.tif'):
            for img_path in glob.glob(os.path.join(self.image_dir, ext)):
                ...
                target = self.extract_multilabel_from_imgpath(img_path)
    ...
    def deterministic_train_val_test_split(self):
        self.train_val_idx, self.test_idx = train_test_split(
            self.all_idx, random_state=2024, test_size=0.15, train_size=0.85
        )
        self.train_idx, self.val_idx = train_test_split(
            self.train_val_idx, random_state=2024, test_size=0.1765, train_size=0.8235
        )
...
class DeepGlobeDataModule(LightningDataModule):
    def __init__(
        self,
        root_path: str,
        batch_size: int,
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_dataset = DeepGlobeDataset(
                dataset_path=self.dataset_path,
                split='train',
                transform=self.train_transform,
            )
            self.val_dataset = DeepGlobeDataset(
                dataset_path=self.dataset_path,
                split='validation',
                transform=self.test_transform,
            )
        if stage == 'test' or stage is None:
            self.test_dataset = DeepGlobeDataset(
                dataset_path=self.dataset_path,
                split='test',
                transform=self.test_transform,
            )
