...
class RSD46WHUDataModule(LightningDataModule):
    def __init__(
        self,
        root_path: str,
        batch_size: int,
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            self.train_dataset = RSD46WHUDataset(
                dataset_path=self.dataset_path,
                split='train',
                transform=self.train_transform,
            )
            self.val_dataset = RSD46WHUDataset(
                dataset_path=self.dataset_path,
                split='validation',
                transform=self.test_transform,
            )
        if stage == 'test' or stage is None:
            self.test_dataset = RSD46WHUDataset(
                dataset_path=self.dataset_path,
                split='test',
                transform=self.test_transform,
            )
